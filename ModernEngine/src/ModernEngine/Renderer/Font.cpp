...
namespace ModernEngine {
		...
		: m_Data(new MSDFData())
	{
		...
		if (ft) {
			...
			if (remaining > 0)
				MN_CORE_ERROR("Failed to pack the glyphs");

			int width, height;
			atlasPacker.getDimensions(width, height);
			emSize = atlasPacker.getScale();

#define DEFAULT_ANGLE_TRESHOLD 3.0
...
#define THREAD_COUNT 8
			...
			if (expensiveColoring) {
				msdf_atlas::Workload([&glyphs = m_Data->Glyphs, &coloringSeed](int i, int threadNo) -> bool {
					...
					glyphs[i].edgeColoring(msdfgen::edgeColoringInkTrap, DEFAULT_ANGLE_TRESHOLD, glyphSeed);
					return true;
				...
				for (msdf_atlas::GlyphGeometry& glyph : m_Data->Glyphs) 
				{
					...
					glyph.edgeColoring(msdfgen::edgeColoringInkTrap, DEFAULT_ANGLE_TRESHOLD, glyphSeed);
				}
			}

			m_AtlasTexture = CreateAndCacheAtlas<uint8_t, float, 3, msdf_atlas::msdfGenerator>("Test", (float)emSize, 
				m_Data->Glyphs, m_Data->FontGeometry, width, height);
