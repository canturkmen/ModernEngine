...
class PythonConfiguration:
    ...
    def __ValidatePython(cls, versionMajor = 3, versionMinor = 3):
        if sys.version is not None:
            print("Python {0:d}.{1:d}.{2:d} detected.".format(sys.version_info.major, sys.version_info.minor, sys.version_info.minor))

            if sys.version_info.major < versionMajor or (sys.version_info.major == versionMajor and sys.version_info.minor < versionMinor ):
                print("Python version is low, expected version is: {0:d}.{1:d}.{2:d} or higher.".format(versionMajor, versionMinor))
                return False
            return True
